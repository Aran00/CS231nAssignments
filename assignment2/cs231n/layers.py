...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    s = x - sample_mean
    q = np.sqrt(sample_var + eps)  # (D,)
    normalized_x = s/q
    out = gamma * normalized_x + beta
    cache = x, s, q, normalized_x, gamma   
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var = momentum * running_var + (1 - momentum) * sample_var
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x, s, q, normalized_x, gamma = cache
  N = dout.shape[0]
  #dx = gamma * (q**-1) * (dout - 1./N * np.sum(dout, axis=0)) + gamma * s ((-np.sum(dout, axis=0)/q**2) * (0.5/q) * (2.0/N)*(1.0-1.0/N)*x)
  dx = gamma * (q**-1) * (dout - 1./N * np.sum(dout, axis=0)) + gamma * -np.sum(dout * s, axis=0) * q**-2 * 0.5 * q**-1 * (2.0/N) * s
  # dx = (1. / N) * gamma * (q**-1) * (N * dout - np.sum(dout, axis=0) - (s) * q**(-2.) * np.sum(dout * s, axis=0))
...
def conv_backward_naive(dout, cache):
  ...
  H_out = 1 + (H + 2 * pad - HH) / stride
  W_out = 1 + (W + 2 * pad - WW) / stride
  dx_padding = np.zeros((N, C, H + 2*pad, W + 2*pad))
  for i in xrange(N):
    for j in xrange(C):
      for m in xrange(H + 2*pad): 
        for n in xrange(W + 2*pad):
          # dx[i, j, m, n] = 0
          for p1 in xrange(F):
            for p2 in xrange(H_out):
              idx_2 = m - p2*stride
              if idx_2 < 0 or idx_2 >= HH:
                continue
              for p3 in xrange(W_out):
                idx_3 = n - p3*stride
                if idx_3 < 0 or idx_3 >= WW:
                  continue
                dx_padding[i, j, m, n] += dout[i, p1, p2, p3] * w[p1, j, idx_2, idx_3]
      ...
      for m in xrange(HH):
        for n in xrange(WW):
          for p0 in xrange(N):
            for p2 in xrange(H_out):
              for p3 in xrange(W_out):
                dw[i, j, m, n] += dout[p0, i, p2, p3] * x_padding[p0, j, m+p2*stride, n+p3*stride]
    ...
    for j in xrange(C):
      for m in xrange(H_out):
        for n in xrange(W_out):
          pooling_area = x[i, j, m*stride:(m*stride+pool_height), n*stride:(n*stride+pool_width)]
          out[i, j, m, n] = np.max(pooling_area)
  ...
  for i in xrange(N):
    for j in xrange(C):
      for m in xrange(H_out):
        for n in xrange(W_out):
          pooling_area = x[i, j, m*stride:(m*stride+pool_height), n*stride:(n*stride+pool_width)]
          p,q = np.unravel_index(np.argmax(pooling_area), pooling_area.shape)
          dx[i, j, m*stride + p, n*stride + q] = dout[i, j, m, n]
