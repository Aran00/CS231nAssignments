...
def rnn_step_backward(dnext_h, cache):
  ...
  dout_temp = dnext_h * (1 - next_h**2)    # (N, H)
  dx = np.dot(dout_temp, Wx.T)
  dprev_h = np.dot(dout_temp, Wh.T)
  dWx = np.dot(x.T, dout_temp)
  dWh = np.dot(prev_h.T, dout_temp)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  dtanh_c = dnext_h * o
  do = dnext_h * tanh_c
  dnext_c_final = dtanh_c * (1 - tanh_c ** 2) + dnext_c
  dprev_c = dnext_c_final * f     # (N, C)
  di = dnext_c_final * g
  dg = dnext_c_final * i
  df = dnext_c_final * prev_c
  da_i = di * i * (1 - i)
  da_f = df * f * (1 - f)
  da_o = do * o * (1 - o)
  da_g = dg * (1 - g**2)
